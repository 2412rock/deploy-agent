...
def deploy_redis_server():
    ...
    subprocess.Popen(["docker", "run", "-d", "-p", "6379:6379", "--name", "redis-server", "-e", f"REDIS_PASSWORD={get_redis_password()}", "redis"], detach=True)
...
def deploy_frontend():
    ...
    subprocess.Popen(["docker", "run", "--name", "angular-app", "-p", "443:443", "dorel-angular"], detach=True)
...
def deploy_backend():
    ...
    subprocess.Popen(["docker", "run" , "-e", f'EMAIL_PASSWD={email_password}', "-e", f'SA_PASSWORD={getSqlPassword()}',
                       ...
                       "-e", f"MINIO_PASS={minio_password}",
                         "--name" ,"dorel-backend", "-p" ,"4200:4200" ,"dorel-backend"], detach=True)
